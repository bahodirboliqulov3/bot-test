...
# Direct single test code sent without state: e.g. "101" or "TEST-105" or "SAT-101" or "TEST-AB12C"
@router.message(StateFilter(None), F.text.regexp(r"(?i)^[#/#]?(TEST-[A-Z0-9_\-]+|SAT-[A-Z0-9_\-]+|[A-Z0-9_\-]{2,24})$"))
async def direct_single_test_code_handler(message: Message, state: FSMContext, session: AsyncSession):
    ...
    info_msg = (
        ...
        f"• <code>A, B, C, 3/4, 0.75, 12</code> <i>(SAT / kasrli / raqamli)</i>\n"
...
@router.message(StateFilter("*"), F.text.in_(["📐 SAT javobini tekshirish", "🔢 SAT javobini tekshirish", "SAT javobini tekshirish", "/sat_check", "/sat"]))
async def start_sat_check_menu(message: Message, state: FSMContext):
    await state.set_state(QuickCheckState.waiting_for_test_code)
    await message.answer(
        "📐 <b>SAT / Raqamli & Kasrli javoblarni tekshirish</b>\n\n"
        "1️⃣ <b>SAT test kodini kiriting:</b>\n"
        "<i>(Masalan: <code>SAT-101</code> yoki <code>101</code>)</i>\n\n"
        ...
        "<code>SAT-101 1.A 2.B 3.12 4.3/4 5.-4.5</code>",
        reply_markup=get_cancel_keyboard(),
        parse_mode="HTML"
    )
...
async def process_quick_check_test_code(message: Message, state: FSMContext, session: AsyncSession):
    ...
    info_msg = (
        ...
        f"• <code>A, B, C, 3/4, 0.75, 12</code> <i>(SAT / kasr / raqam)</i>\n"
